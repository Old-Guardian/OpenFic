...
async def test_agent_queries_character_relationship_paths(revision_db):
    from app.agent_runtime.tools.impls.context.character import (
        ListCharactersTool,
        QueryCharacterRelationshipsTool,
        ReadCharacterTool,
    )
    from app.storage.services import character_relationship_service
    ...
    listing = json.loads(await ListCharactersTool(_state=state).ainvoke({}))
    listed_character = next(item for item in listing["characters"] if item["id"] == "char-1")
    assert listed_character["relationships"][0]["character_name"] == "中转"
