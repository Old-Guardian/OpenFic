"""Add character relationships and graph positions after the fork's 1025 head.

Revision ID: 1026
Revises: 1025

The upstream migration used revision 1023, which already identifies the fork's
agent reasoning effort migration. Keep the deployed fork history intact.
"""
...
def upgrade() -> None:
    ...
    for column in ("project_id", "source_character_id", "target_character_id"):
        op.create_index(
            f"ix_character_relationships_{column}",
            "character_relationships",
            [column],
        )
    op.create_table(
    ...
    for column in ("revision_id", "relationship_id", "project_id"):
        op.create_index(
            f"ix_revision_character_relationship_snapshots_{column}",
            "revision_character_relationship_snapshots",
            [column],
        )
